...
def test_pause_video_play_video(capfd):
    ...
    assert "Playing video: Amazing Cats" in lines[0]
    assert "Pausing video: Amazing Cats" in lines[1]
    assert "Playing video: Amazing Cats" in lines[2]
    assert "Currently playing: Amazing Cats (amazing_cats_video_id) [#cat #animal]" in lines[3]
    assert "PAUSED" not in lines[3]
